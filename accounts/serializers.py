...
class HikerRegisterFormSerializer(serializers.Serializer):
...
class HikerLoginFormSerializer(serializers.Serializer):
    email = serializers.EmailField(required=False, allow_blank=True)
    password = serializers.CharField(style={'input_type': 'password'})

    def authenticate_util(self, **kwargs):
    ...
    def _validate_email(self, email, password):
        ...
        if email and password:
            user = self.authenticate_util(email=email, password=password)
        else:
